...
function validateAndSubmit() {
    ...
    if (nombre.trim() === "" || apellidos.trim() === "" || mensaje.trim() === "") {
      // Muestra un mensaje de error si algún campo está vacío
      Swal.fire({
        title: 'Error',
        text: 'Por favor, completa todos los campos.',
        icon: 'error',
        confirmButtonColor: '#00A149'
      });
    } else {
  ...
  function submitWithNotification() {
    Swal.fire({
      title: 'Gracias por tus recomendaciones!',
      text: 'Mensaje enviado con éxito. Serás redirigido a PLAFIME.',
      icon: 'success',
      confirmButtonColor: '#00A149',
      onClose: () => {
        window.scrollTo(0, 0);
      ...
      if (result.isConfirmed) {
        window.location.href = "https://plafime.github.io/plafime"; // Reemplaza con la URL a la que deseas redirigir
      }
    });
  }
...
//   }
  ...
  for (var i = 0; i < iframes.length; i++) {
    iframes[i].onload = function() {
      var video = this.contentWindow.document.querySelector("video");
      video.muted = true;
    };
  }